...
function PizzaBlock({ title, price, imageUrl, types, sizes }) {
  ...
  const [typeChoosen, setTypeChoosen] = React.useState(0);
  const [pizzaSize, setPizzaSize] = React.useState(0);

  const addOnClick = () => {
    setPizzaCount(pizzaCount + 1);
    dispatch(addItem({ title, price, imageUrl, types, sizes }));
  };
    ...
    <div className="pizza-block">
      ...
      <div className="pizza-block__selector">
        <ul>
          {types.map((type) => (
            <li
              onClick={() => setTypeChoosen(type)}
              className={typeChoosen === type ? 'active' : ''}>
              {pizzaType[type]}
            </li>
          ))}
        </ul>
